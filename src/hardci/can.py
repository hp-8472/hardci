...
class CanBusService:
    ...
    def session_start(self, bus_id: str, clear_rx_queue: bool = True) -> JsonObject:
        ...
        if not self.config.permissions.allow_can_read and not self.config.permissions.allow_can_write:
            return self._write_report(self._permission_denied("can_session_start", "CAN reading and writing are disabled by .hardci/config.yaml.", bus_id))
    ...
    def send(self, bus_id: str, payload: JsonObject) -> JsonObject:
        ...
        if not self.config.permissions.allow_can_write:
            return self._write_report(self._permission_denied("can_send", "CAN writing is disabled by .hardci/config.yaml.", bus_id))
    ...
    def read(self, bus_id: str, max_frames: object | None = None, wait_timeout_s: object = 0.0) -> JsonObject:
        ...
        if not self.config.permissions.allow_can_read:
            return self._write_report(self._permission_denied("can_read", "CAN reading is disabled by .hardci/config.yaml.", bus_id))
...
def open_python_can_adapter(config: HardCIConfig, bus_id: str, bus_config: CanBusConfig, clear_rx_queue: bool) -> JsonObject:
    ...
    try:
        ...
        return {"ok": False, "tool": "can_session_start", "bus_id": bus_id, "adapter": bus_config.adapter, "error_type": "can_backend_not_available", "summary": "python-can is not installed. Install hardci[can] to use direct CAN adapters."}
    try:
